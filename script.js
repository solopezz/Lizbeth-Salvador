...
function renderGuest(){
  ...
  if(inviteId !== 'DEMO') gateGuest.textContent = `Invitación para ${guest.name}`;
...
function openInvitation(){
  ...
  setTimeout(()=>{cover.classList.add('is-open');document.body.classList.remove('locked');window.scrollTo({top:0,behavior:'instant'});},950);
}
openInviteButton.addEventListener('click',openInvitation);

// El reproductor visual queda preparado para integrar una fuente de audio autorizada después.
...
form.addEventListener('submit',async event=>{
  ...
  finally{submitButton.disabled=false;submitButton.textContent='Confirmar aquí';}
});
